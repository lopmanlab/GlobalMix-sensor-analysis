...
in.part.ext <- in_part_full%>%
  select(rec_id, participant_age, participant_sex)

## Model
in.method.bin <- in.rel.method.comp%>%
  ...
  left_join(in.part.ext, by = c("contact_id" = "rec_id"))%>%
  rename(contact_age = participant_age,
         contact_sex = participant_sex)%>%
  left_join(in.part.ext, by = c("participant_id" = "rec_id"))%>%
...
in.log.diary.model <- glm(diary_bin ~ participant_age + participant_sex + contact_age + contact_sex + inferred_relationship,
                 data = in.method.bin,
                 family = binomial)

in.log.sensor.model <- glm(sensor_bin ~ participant_age + participant_sex + contact_age + contact_sex + inferred_relationship,
                          data = in.method.bin,
                          family = binomial)

summary(in.log.diary.model)
exp(coef(in.log.diary.model))
exp(confint(in.log.diary.model))

summary(in.log.sensor.model)
exp(coef(in.log.sensor.model))
exp(confint(in.log.sensor.model))
