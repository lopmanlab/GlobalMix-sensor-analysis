...
mo.log.diary.model <- glm(diary_bin ~ participant_age + participant_sex + contact_age + contact_sex + inferred_relationship,
                          data = mo.method.bin,
                          family = binomial)

mo.log.sensor.model <- glm(sensor_bin ~ participant_age + participant_sex + contact_age + contact_sex + inferred_relationship,
                           data = mo.method.bin,
                           family = binomial)

summary(mo.log.diary.model)
exp(coef(mo.log.diary.model))
exp(confint(mo.log.diary.model))

summary(mo.log.sensor.model)
exp(coef(mo.log.sensor.model))
exp(confint(mo.log.sensor.model))
